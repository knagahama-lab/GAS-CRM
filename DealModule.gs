...
function createDeal(data) {
  return wrapAction(() => {
    ...
    // バリデーション
    const valErr = validateRequired(data, ['deal_name', 'customer_id']);
    if (valErr) return errorResponse(valErr.error);

    // サニタイズ
    data.deal_name = sanitizeString(data.deal_name);
...
function updateDeal(id, data) {
  return wrapAction(() => {
    ...
    if (data.deal_name !== undefined) data.deal_name = sanitizeString(data.deal_name);
...
/**
 * 検索・フィルタ・ソート・ページネーション付き商談一覧
 */
function getDealsFiltered(params) {
  return wrapAction(() => {
    requireFeature('FEATURE_DEALS');
    const { customerId, status, minAmount, maxAmount, sortKey, sortAsc, page, pageSize } = params || {};
    ...
    if (customerId) {
      data = data.filter(r => r.customer_id === customerId);
    }
    if (status) {
      data = data.filter(r => r.phase === status);
    }
    if (minAmount !== undefined && minAmount !== '') {
      data = data.filter(r => Number(r.amount) >= Number(minAmount));
    }
    if (maxAmount !== undefined && maxAmount !== '') {
      data = data.filter(r => Number(r.amount) <= Number(maxAmount));
    }
    if (sortKey) {
...
function getPipelineSummary() {
  return wrapAction(() => {
    ...
    phases.forEach(p => {
      ...
      const totalProb = items.reduce((s, r) => s + (Number(r.probability) || 0), 0);
      summary[p] = {
        count: items.length,
        total_amount: items.reduce((s, r) => s + (Number(r.amount) || 0), 0),
        weighted_amount: items.reduce((s, r) => s + (Number(r.weighted_amount) || 0), 0),
        avg_probability: items.length > 0 ? Math.round(totalProb / items.length) : 0,
      };
    });
